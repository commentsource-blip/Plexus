...
def side_frivillig(data:dict):
    ...
    sel_mk=st.selectbox("📅 Vælg periode",alle,index=0,format_func=mk_label,key="vol_month_select")
    st.markdown("")
...
def _vis_praeference(data:dict,vid:str,vol:dict,mkey:str):
    ...
    if cb.button("💾 Gem",type="primary",use_container_width=True,disabled=not nok,key="gem_prefs"):
...
def _tab_frivillige(data:dict):
    ...
    for vid,vol in sorted(data["volunteers"].items(),key=lambda x:x[1]["name"]):
        ...
        q=c2.number_input("",1,20,vol.get("required_shifts",2),key=f"q_{vid}",label_visibility="collapsed")
...
def _tab_resultater(data:dict):
    ...
    mkey=st.selectbox("Vælg måned",tildelte,format_func=mk_label,key="resultater_month_select")
    asgn=data["assignments"][mkey]
    vols=data["volunteers"]
